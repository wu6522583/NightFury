...
+function ($) {
  ...
  var Panel   = function (el,options) {
    ...
    $.ajax({url: options.href,cache: false}).success(function(data){self.html(data);options.onload(data);var reg = new RegExp("html", "g");var strJs = options.href.replace(reg, "js");seajs.use( strJs );});
  }
  var old = $.fn.panel
  $.fn.panel = function (option) {
    return this.each(function () {
      ...
      if( data && data.options.href != options.href ){
        page.push( data.options.href , $this.children()[0]);
